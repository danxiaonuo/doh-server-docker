...
# 镜像变量
ARG DOCKER_IMAGE=danxiaonuo/doh-server
...
WORKDIR /build
# 克隆源码运行安装
RUN git clone --depth=1 --progress https://github.com/m13253/dns-over-https.git /src && \
    cd /src && make
# ##############################################################################
...
# 拷贝doh-server
COPY --from=0 /src/doh-server/doh-server /usr/bin/doh-server

# 授予文件权限
RUN set -eux && \
    mkdir -p /etc/doh-server && \
    chmod +x /usr/bin/doh-server

# 增加配置文件
COPY --from=0 /src/doh-server/doh-server.conf /etc/doh-server/doh-server.conf

# 修改端口号
RUN sed -i '$!N;s/"127.0.0.1:8053",\s*"\[::1\]:8053",/":8053",/;P;D' /etc/doh-server/doh-server.conf
